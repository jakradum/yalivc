  ...
  try {
    ...
    if (action === 'send-code') {
      ...
      const domainPriv = await client.fetch(
        `*[_type == "domainPrivilege" && domain == $domain && dataroomAccess == true && isActive == true][0]{
...
<div class="wrapper">
  ...
  <div class="body">
    <p class="greeting">Enter the code below to sign in to the Yali Capital Data Room. This code is valid for 24 hours and can only be used once.</p>
  ...
  <div class="footer">
    ...
    if (action === 'verify-code') {
      ...
      if (email) {
        ...
        const domain = normalizedEmail.split('@')[1];
        const domainPriv = await client.fetch(
          `*[_type == "domainPrivilege" && domain == $domain && dataroomAccess == true && requireCode == true && isActive == true][0]{
    ...
    return NextResponse.json({ error: 'Authentication failed', detail: error?.message }, { status: 500 });
  }
}
